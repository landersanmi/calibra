...
def run_speed_test(result, i):
    s = speedtest.Speedtest()
    s.download(threads=None)
    s.upload(threads=None)
    s.results.share()
    print(s.results.dict())
    result[i] = s.results.dict()

def get_energy_efficiency():
    throughputs = []
    energy_efficiencies = []
    for i in range(0, 5):
        x = threading.Thread(target=run_speed_test, args=(throughputs, i))
        x.start()
        p_powerstat = subprocess.Popen(['powerstat', '-R', '-c', '-z'],
            stdout=subprocess.PIPE,
            stderr=subprocess.PIPE)
        ...
        if (p_powerstat.returncode != 0):
            return None
        consumption = float(re.search('CPU:\s+(\d+.\d+) Watts on average', stdout.decode('utf-8'))[1])*electricity_price
        energy_efficiencies.append(throughputs[i]/consumption)
    return np.mean(energy_efficiencies.values)
...
def main():
    ...
    required.add_argument('-e', '--energy', action='store_true',
                          help='Energy Eficiency metric', required=False)
    ...
    if args.energy:
        ...
        logging.info(f"Energy Efficiency = {energy_efficiency}")
